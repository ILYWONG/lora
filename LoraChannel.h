...
LoraChannel::channel
{
  ...
  Ptr<NetDeviceContainer> LoraNetDevicecontainer;
	Time m_delay;//时延信息
  Ptr<Packet>m_currentPkt;//信道中的当前包，暂时不用
 
	public:
	void LoraChannel();//构造函数
	bool TransmitStart (Ptr<Packet> p, uint32_t srcId); //核心的发包函数
	bool init_set_netdevice_lora(Ptr<NodeContainer>devices_trans);//设置信道中的网络设备
	...
	bool set_current_packet(Ptr<Packet> read); //这个要实现，但是要注意同步与异步的问题
}
;